...
<head>
    <title>
        <fmt:message bundle="${loc}" key="news.title_text"/>
    </title>
        ...
        <section>
            ...
            <div class="page_block wide_block post_content">
                <div class="page_main_header_block">
                    <h1>
                        <fmt:message bundle="${loc}" key="news.main_title_text"/>
                    </h1>
                </div>
            </div>
                ...
                <c:if test="${requestScope.posts.getTotalPagesCount() > 1}">
                    ...
                    <ul class="pagination">
                        <c:if test="${requestScope.posts.getCurrentPage() > 1}">
                            <li>
                                <a href="${go_to_my_news}${page_no}${requestScope.posts.getCurrentPage()-1}">«</a>
                            </li>
                        </c:if>
                        <c:forEach var="number" begin="1" end="${requestScope.posts.getTotalPagesCount()}">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <li>
                                        <a href="${go_to_my_news}${page_no}${number}">${number}</a>
                                    </li>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
