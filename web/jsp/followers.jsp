...
<head>
    <meta charset="utf-8">
    <title>
        <fmt:message bundle="${loc}" key="followers.title_text"/>
    </title>
            ...
            <div class="wall_content wide_block">
                <div class="page_block wide_block post_content">
                    <div class="page_main_header_block">
                        <h1>
                            <fmt:message bundle="${loc}" key="followers.main_title"/>
                        </h1>
                    </div>
                </div>
                <c:choose>
                        ...
                        <div class="page_block wide_block post_content">
                            <div class="page_main_header_block">
                                <div class="no_friends">
                                    <fmt:message bundle="${loc}" key="followers.txt.no_friends"/>
                                </div>
                            </div>
                        </div>
                    </c:when>
                        ...
                        <c:if test="${requestScope.friends.getTotalPagesCount() > 1}">
                            ...
                            <ul class="pagination">
                                <c:if test="${requestScope.friends.getCurrentPage() > 1}">
                                    <li>
                                        <a href="${go_to_followers}${page_no}${requestScope.friends.getCurrentPage()-1}">«</a>
                                    </li>
                                </c:if>
                                <c:forEach var="number" begin="1" end="${requestScope.friends.getTotalPagesCount()}">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <li>
                                                <a href="${go_to_followers}${page_no}${number}">${number}</a>
                                            </li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
